...
void PointShadow::initialize(int resolution) {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "PointShadow Framebuffer not complete!" << std::endl;
    }
...
void PointShadow::destroy() {
    if (m_depthMapFBO != 0) CHECKED_GL_CALL(glDeleteFramebuffers, 1, &m_depthMapFBO);
    if (m_depthCubemap != 0) CHECKED_GL_CALL(glDeleteTextures, 1, &m_depthCubemap);
    m_initialized = false;
}
